...
import { getDb, SCHOOL_ID, isFirebaseReady } from './firebase';
...
function col(path: string) {
  return collection(getDb(), 'schools', SCHOOL_ID, path);
}
function docRef(path: string, id: string) {
  return doc(getDb(), 'schools', SCHOOL_ID, path, id);
}
...
export async function fsSaveSettings(data: Record<string, unknown>) {
  if (!guard()) return;
  await setDoc(doc(getDb(), 'schools', SCHOOL_ID), { ...data, updatedAt: serverTimestamp() }, { merge: true });
}

export function fsListenSettings(cb: (data: DocumentData) => void): Unsubscribe {
  if (!guard()) return () => {};
  return onSnapshot(doc(getDb(), 'schools', SCHOOL_ID), snap => {
